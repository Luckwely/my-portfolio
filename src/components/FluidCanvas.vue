<template>
    <canvas ref="canvasRef" class="fixed top-0 left-0 w-screen h-screen z-0 pointer-events-none" />
</template>
...
const props = defineProps({
    ditheringTexturePath: {
        type: String,
        default: '/images/LDR_LLL1_0.png',
    },
})
...
function initFluid(canvas) {
    ...
    console.log('FluidCanvas: canvas', canvas, 'clientWxH', canvas ? canvas.clientWidth : null, canvas ? canvas.clientHeight : null)
    console.log('FluidCanvas: gl', !!gl, 'isWebGL2', gl && gl instanceof WebGL2RenderingContext, 'ext', ext)
  ...
  `)
    ...
    const ditheringTexture = createTextureAsync(props.ditheringTexturePath)
    ...
    updateKeywords()
    initFramebuffers()
    splat(0.5, 0.5, config.SPLAT_FORCE, config.SPLAT_FORCE, generateColor())

    let lastUpdateTime = Date.now()
    let colorUpdateTimer = 0.0

    // start loop after timers are initialized
    update()
    ...
    // ─── Event listeners (jQuery-free, scoped, cleaned up on unmount) ─────────
    ...
    // One-time mousemove that also starts the loop — native version of $('body').one(...)
    ...
    addListener(window, 'touchstart', e => {
        ...
        for (let i = 0; i < touches.length; i++) {
            ...
            if (firstTouchStart) { firstTouchStart = false; update() }
